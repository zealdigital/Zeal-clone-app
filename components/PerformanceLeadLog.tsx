...
// ✅ FIX 1: Corrected import name from formatToDDMMYY → formatDDMMYY (matches what dateUtils actually exports)
...
// ✅ FIX 2: Inline SVG sort icons — removes dependency on ArrowUpIcon/ArrowDownIcon
// which may not be exported from Icons.tsx
...
const PerformanceLeadLog: React.FC<PerformanceLeadLogProps> = ({
  ...
  const uniqueCallers = useMemo(() => {
    ...
    mappedBookings.forEach(b => {
      if (isVendorRole) {
        if (b.callerName) callers.add(b.callerName);
      } else {
        if (b.vendor?.name) callers.add(b.vendor.name);
      }
    });
  ...
  // ✅ FIX 3: Inline sort icon component — no longer depends on missing Icons exports
  ...
  const allFilteredLeads = useMemo(() => {
    const filtered = mappedBookings.filter(b => {
      ...
      const matchesBdm =
        isVendorRole || isBdmRole || bdmFilter === 'all' ||
        (b.bdmId?.toString() === bdmFilter);
    ...
    // ✅ FIX 4: Wrapped case blocks in {} to prevent "lexical declaration in case clause" TS error
  ...
  const getHistoryBadge = (lead: Booking) => {
    if (lead.status === 'rescheduled_bdm') {
      return (
        <span className="px-2 py-1 bg-orange-100 text-orange-600 rounded font-black text-[9px] uppercase tracking-tighter">
          BDM Reschedule
        </span>
      );
    }
    if (lead.isDuplicate && (new Date(lead.date) >= new Date(new Date().setFullYear(new Date().getFullYear() - 1)))) {
      return (
        <span className="px-2 py-1 bg-red-100 text-red-600 rounded font-black text-[9px] uppercase tracking-tighter">
          Duplicate Lead
        </span>
      );
    }
    if (
      lead.customReason?.toLowerCase().includes('manual') ||
      lead.customReason?.toLowerCase().includes('request')
    ) {
      return (
        <span className="px-2 py-1 bg-purple-100 text-purple-600 rounded font-black text-[9px] uppercase tracking-tighter">
          Rebooked/Manual
        </span>
      );
    }
    return (
      <span className="px-2 py-1 bg-green-100 text-green-600 rounded font-black text-[9px] uppercase tracking-tighter">
        Fresh Lead
      </span>
    );
  };
  ...
  // Column span for the empty-results row
  const emptyColSpan = isVendorRole ? 6 : (isBdmRole ? 6 : 7);
      ...
      <div className="p-8 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
            {isVendorRole
              ? 'Detailed breakdown of leads you have booked'
        ...
        {!hideFilters && (
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Caller Filter — hidden for vendors */}
            ...
            {/* BDM Filter — hidden for vendors and BDMs */}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50/50">
            <tr>
              ...
              <th scope="col" className="px-6 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">
                Business, Client & URL
              </th>

              <th scope="col" className="px-6 py-5 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">
                Lead History & Source
              </th>
              ...
              {/* Assigned BDM — sortable, managers only */}
              {!isVendorRole && !isBdmRole && (
                <th
                  scope="col"
                  className="px-6 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest cursor-pointer group hover:text-gray-600 transition-colors"
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {paginatedLeads.length > 0 ? (
                ...
                return (
                  <tr key={lead.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-6 whitespace-nowrap text-sm font-medium text-gray-500">
                      {formatDDMMYY(lead.date)}
                      {lead.createdAt && (
                        <div className="text-[9px] text-gray-300 mt-0.5">
                          Created: {formatDDMMYY(lead.createdAt)}
                        </div>
                      )}
                    </td>
              ...
              <tr>
                <td colSpan={emptyColSpan} className="px-6 py-24 text-center">
      ...
      <div className="p-6 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest order-2 sm:order-1">
          ...
          {allFilteredLeads.length > 0
            ? Math.min(allFilteredLeads.length, (currentPage - 1) * ITEMS_PER_PAGE + 1)
            : 0}{' '}
          to {Math.min(allFilteredLeads.length, currentPage * ITEMS_PER_PAGE)} of {allFilteredLeads.length} total leads
        </span>
