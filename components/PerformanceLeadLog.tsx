...
const PerformanceLeadLog: React.FC<PerformanceLeadLogProps> = ({ bookings, title = "Lead Performance Log" }) => {
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mt-8 shadow-sm">
      <div className="p-6 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex-shrink-0">
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Cross-team performance insights</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50/80">
            <tr>
              ...
              <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Caller</th>
              <th scope="col" className="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Region</th>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {filteredLeads.length > 0 ? filteredLeads.map((lead) => (
              <tr key={lead.id} className="hover:bg-indigo-50/40 transition-colors group">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="px-2 py-0.5 rounded-full bg-white text-gray-500 text-[10px] font-black border-2 border-gray-100 uppercase tracking-tight">
                    {lead.region}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
              ...
              <tr>
                <td colSpan={5} className="px-6 py-20 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-gray-400 font-bold italic">No matching leads found for these filters.</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      <div className="p-4 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center">
        <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
          Displaying {filteredLeads.length} of {bookings.length} database entries
        </span>
      </div>
    </div>
  );
